...
zstyle ':completion:*:descriptions' format %F{default}%B%{$__JOAN[ITALIC_ON]%}--- %d ---%{$__JOAN[ITALIC_OFF]%}%b%f
...
function () {
  ...
  local TMUXING=$([[ "$TERM" =~ "tmux" ]] || [[ "$TERM" =~ "screen" ]] && echo tmux)
...
function -report-start-time() {
  ...
  if [ $ZSH_START_TIME ]; then
    ...
    export RPROMPT="%F{cyan}%{$__JOAN[ITALIC_ON]%}${ELAPSED}%{$__JOAN[ITALIC_OFF]%}%f $RPROMPT_BASE"
