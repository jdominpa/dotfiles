...
#+begin_src emacs-lisp
  (use-package jdp-fonts
    ...
    (setq jdp-fonts-typeface-sets-alist
          '((laptop 160 "Fira Code" normal "DejaVu Sans Condensed" normal)
...
I used to use Ivy but Emacs version 28 comes with a way to display
=*Completions*= in a vertical list, one item per line.  Using a
handful of functions written by Protesilaos (see his [[https://protesilaos.com/dotemacs/][dotfiles]] for more
information) I have extended the minibuffer's default completions
capabilities to a point I am satisfied with.  These extensions can be
found in the =jdp-minibuffer.el= file reproduced below, while the
properties of the =*Completions*= buffer is controlled by the
=display-buffer-alist= (see Window rules and basic tweaks). *TODO*

What my completion setup does is the follwing:

+ When minibuffer completion is invoked, a live updating
  =*Completions*= buffer is placed right above the minibuffer
  displaying all completion candidates in a vertical list with meta
  data provided by Marginalia (see [[#completion_annotations][Completion annotations
  (marginalia)]]).  This only happens after some input has been
  provided, otherwise all you see is an empty minibuffer.

  - The length of minimium input that triggers live updating
    completion is controlled by the variable
    =jdp-minibuffer-minimum-input=.

  - =jdp-minibuffer-live-update-delay= controls how fast the live
    updating completion should respond to minibuffer input.

  - The variables =jdp-minibuffer-completion-blocklist= and
    =jdp-minibuffer-completion-passlist= can be used to disable live
    completion for specific commands or bypass the minimum input
    threshold and the delay.

+ Using the usual motions =C-n= and =C-p= we can switch from the
  minibuffer to the completions and vice versa treating the two areas
  as a contiguous space.

+ When completing file names, the current path will be hidden by
  specifying a base directory either of the user's =$HOME= or the file
  system's absolute root.  In other word, =~/= or =/= followed by any
  valid candidate will clear the minibuffer's contents and continue
  the completion with respect to the new path.

+ The =M-backspace= deletes a character by default unless it finds a
  directory signified by =/= in which case, it will remove the entire
  directory.

+ While in the minibuffer or the =*Completions*= buffer, we can pick a
  completion candidate with =jdp-minibuffer-choose-completion-number=
  which takes a numeric prefix and selects the candidate that
...
#+begin_src emacs-lisp
  (use-package jdp-minibuffer
    :demand t
    :hook ((completion-list-mode . jdp-minibuffer-hl-line)
           (completion-list-mode . jdp-minibuffer-display-line-numbers))
    :bind (:map completion-list-mode-map
           ("M-g" . jdp-minibuffer-choose-completion-number)
           ("M-v" . jdp-minibuffer-focus-minibuffer)
           ...
           ("<M-backspace>" . jdp-minibuffer-backward-updir))
    :config
    (setq jdp-minibuffer-remove-shadowed-file-names t)
    (setq jdp-minibuffer-minimum-input 3
          jdp-minibuffer-live-update-delay 0.3)
    (setq jdp-minibuffer-completion-passlist nil
          jdp-minibuffer-completion-blocklist nil))
...
*** TODO Enhanced minibuffer commands (consult)
:PROPERTIES:
...
*** Extended minibuffer actions and more (embark)
:PROPERTIES:
:CUSTOM_ID: embark
:END:

Embark a unified fraemwork of Emacs keymaps which lets us carry out
/contextually relevant actions/ on *targets* through a prefix key.

+ Actions are standard Emacs commands like =describe-symbol= or other
  interactive commands that read arguments from the minibuffer.

+ Targets are semantically sensitive constructs such as the symbol at
  point, a file name, the active region, the current completion
  candidate in the minibuffer, etc.
  
+ The contextually relevant actions are defined in keymaps whose scope
  matches the category of the target.  For example, =embark-file-map=
  holds all keys and commands assossiated with a file name target.

+ The prefix key is an Embark command such as =embark-act= or
  =embark-become=.

Embark can act on individual targets or sets of targets.  To learn
more about the available keymaps we can use =M-x describe-keymap= and
then search for =embark=.

Here are some actions that we can gain access to by invoking either
=embark-act=, or =embark-become=:

+ To learn which keymap's contents get enabled in the present context,
  follow up the =embark-act= call with =C-h=.

  - Keymaps aside, we can call any command after invoking
    =embark-act=. This can be either with =M-x= or via a key
    binding. For example, if you want to grep for the symbol at point
    in the current project, you can do embark-act and then =C-x p g=.

+ We can collect entire set of targets and store them in a buffer
  which we can then re-use or save to disk.  This can be done with the
  =embark-collect-snapshot= command, which we can access through
  =embark-act=.

  - The =*Embark Collect*= buffer can be presented as a grid or a
    list, with the possibility of manually switching between the two
    with the =embark-collect-toggle-view= command.  The list view has
    space to display annotations (see [[#completion_annotations][Completion annotations
    (marginalia)]]) like the first line of a variable's doc string and
    current value.

  - =*Embark Collect*= buffer also has a live-updating version which
    can be used to filter the list of targets.

  - In addition to beign able to produce a snapshot, Embark can also
    collect the targets in a buffer whose major mode is specialised to
    work on the category that those targets belong to.  This can be
    done with the =embark-export= command.  For example, if we are
    targeting files, then the export takes them to a =dired-mode=
    buffer; buffers go to =ibuffer-mode=; grep results go to
    =grep-mode=, and so on.
...
The =display-buffer-alist= variable is used to define window rules to
control the place and behaviour of them.  It is mainly used with
auxiliary buffers such as shells, compilation output, etc.  The idea
is to create an intuitive workflow where specific buffer groups or
types are always shown at a given location to improve predictability.

For each buffer in =display-buffer-alist= we can define several
functions for selecting the appropriate window.  These functions will
be executed in sequence.

Information about buffer actions can be found at the GNU Emacs Lisp
Reference Manual, using =C-h f display-buffer= and with =C-h f
...
*** TODO Directional window motions (windmove)
:PROPERTIES:
:CUSTOM_ID: directional_window_motions
:END:

*TODO* currently trying ace-window

Windmove is also built into Emacs. It provides functions for selecting
a window in any of the cardinal directions. It also has commands for
deleting a window in the given direction as well as for switching the
current window with the one in the given direction. I do not need the
directional deletion motions, as they require extra key bindings while
I feel that it is easy enough to select a window and delete it
...
#+begin_src emacs-lisp
  ...
  ;;   :demand t
  ;;   :bind (("C-x C-<up>" . windmove-up)
  ;;          ("C-x C-<right>" . windmove-right) ; override `next-buffer'
  ;;          ("C-x C-<down>" . windmove-down)
  ;;          ("C-x C-<left>" . windmove-left)   ; override `previous-buffer'
...
only the first one as I don not use the likes of =aansi-term= or
=vterm=.
