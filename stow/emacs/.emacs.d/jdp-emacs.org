...
#+begin_src emacs-lisp
  (use-package jdp-simple
    ...
    (define-key global-map (kbd "C-S-n") #'jdp-simple-multi-line-next)
    (define-key global-map (kbd "C-S-p") #'jdp-simple-multi-line-prev)
    ...
    (define-key global-map (kbd "C-=") #'count-words))
#+end_src
...
#+begin_src emacs-lisp
  (use-package jdp-fonts
    ...
    (setq jdp-fonts-typeface-sets-alist
          '((laptop 140 "Hack" normal "DejaVu Sans Condensed" normal)
            (desktop 130 "Hack" normal "Inter" normal)
    ...
    (define-key global-map (kbd "C-c f") #'jdp-fonts-set-fonts-dwim))
#+end_src
...
(also read *Dired (directory editor, file manager)*).
...
style is append to the minibuffer's customisation options for
...
It defines three style dispatchers.  Those are single characters that
...
Also check my setup for the Minibuffer history (savehist-mode).
...
#+begin_src emacs-lisp
  (use-package minibuffer
    ...
    ;; Some keybindings for *Completions* buffer.
...
#+begin_src emacs-lisp
  ...
  (use-package jdp-search
    :config
    (define-key isearch-mode-map (kbd "<backspace>") #'jdp-search-isearch-abort-dwim))
#+end_src
