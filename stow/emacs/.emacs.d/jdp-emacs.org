...
Finally, I would like to make point out that a lot of this
configuration is inspired (or copied directly from) by [[https://protesilaos.com/dotemacs/][Protesilaos
Stavrou configuration]] also done in Org.  If you check his set up
you'll find many similarities with mine.  If you want to learn about
Emacs, politics and philosophy definetly check out his [[https://www.youtube.com/channel/UC0uTPqBCFIpZxlz_Lv1tk_g][youtube channel]]
and his [[https://protesilaos.com/][website]] (check other people's Emacs work in [[#other_people_emacs_work][this section]]).
...
=init.el=. Therefore, from version 27.1 onwards, package
initialization is enabled by default and done after loading
=early-init.el= but before loading =init.el=. Hence, calling
=package-initialize= is no longer needed.
...
:PROPERTIES:
:CUSTOM_ID: expand_region
:END:

=expand-region= is a package that implements incremental region
selection by semantic units.  For example, let's say you have the
...
When you press the key bound to =er/expand-region= the word =abc= will
be marked.  Pressing the keybind a second time will expand the region
marked to the content inside quotes.  A third press will expand to the
entire quote ="abc def"=.  Then the contents of the sexp =setq
alphabet-start "abc def"= will be marked and finally the whole sexp.

I use the key =C-== to call =er/expand-region=.
...
literal, flex, initialism, etc.  These styles are internally now as
dispatchers.  Being an Emacs package, you can customize the delimiters
used to determine components and the dispatchers used for the patter
matching.
...
extras]].  My set up is based on a macro which I have outright stolen
from Omar called =dispatch:=.  Given a regular expression and an
orderless style, this macro creates a dispatcher named
=dispatch:style= which applies the given style to all components
matching the regular expression.  In my configuration I use the macro
to define dispatchers for various styles, some of which are: literals,
regular expressions, prefixes, initialisms, etc.  In addition, I set
up orderless to separate components by spaces and + signs.
...
:PROPERTIES:
:CUSTOM_ID: minibuffer_history
:END:

The built-in =savehist-mode= keeps a record of previous actions
involving the minibuffer.  Those inputs can be accessed with =M-p= and
=M-n=.  Well designed commands that use completion have their own
history, which means that their record won't be mixed up with the ones
from other commands.  You can also search backwardly with =M-r=.

Emacs will remember your input and choices so it will prioritise the
desired results when trying to complete an input.
...
This is Emacs' own completion system for in-buffer text.  By default
=dabbrev-expand= tries to complete the text at point.  Repeated
invocations of the command will cycle through the candidates.  With
Corfu, the completion candidates are displayed in a pop-up window near
point ([[#corfu][Corfu for in-buffer completion]]).
